...
int ssl_write_entry(struct pt_regs *ctx) {
    ...
    // Extract file descriptor from SSL context (for connection tracking)
    __u64 fd = 0;
    // Note: We'll get the actual FD from the syscall stack later
    
    // Call our existing write tracing logic with TLS flag
    return trace_enter_write(ctx, fd, 1, buf, num, 0); // is_tls = 1
}

SEC("uprobe/ssl_read_manual") 
int ssl_read_entry(struct pt_regs *ctx) {
    ...
    // Store read arguments for the return probe
    __u64 id = bpf_get_current_pid_tgid();
    __u32 pid = id >> 32;
    __u64 fd = 0; // Will be resolved from syscall stack
...
int ssl_read_exit(struct pt_regs *ctx) {
    ...
    // Call our existing read exit logic with TLS flag  
    return trace_exit_read(ctx, id, pid, 1, ret); // is_tls = 1
}

// GnuTLS support for broader SSL library coverage
SEC("uprobe/gnutls_write_manual")
int gnutls_write_entry(struct pt_regs *ctx) {
    ...
    __u64 fd = 0; // Will be resolved from connection tracking
    return trace_enter_write(ctx, fd, 1, buf, size, 0); // is_tls = 1
}

SEC("uretprobe/gnutls_read_manual") 
